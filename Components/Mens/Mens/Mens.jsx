"use client"; // تأكد من أن هذا في أعلى الملف
import { useSelector } from 'react-redux';
import { selectMensProducts } from '../../../store/mensSlice'; // تأكد من استيراد الشريحة الصحيحة
import { ShoppingCartIcon, EyeIcon, ChevronLeftIcon, ChevronRightIcon } from '@heroicons/react/24/solid';
import { useState } from 'react';

export default function Mens() {
    const products = useSelector(selectMensProducts); // استخدام Redux لاستخراج المنتجات الرجالية
    ...
    return (
        <div className="flex flex-col items-center px-4 md:px-8 pt-8">
            <h1 className="text-3xl font-bold mb-2 text-left w-full max-w-6xl">Mens Latest</h1>
            <p className="text-lg text-gray-700 mb-4 w-full max-w-6xl text-left">
                Discover the latest trends in men's fashion. Shop our newest arrivals and find the perfect outfit!
            </p>
                            ...
                            <div className="relative group">
                                ...
                                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black bg-opacity-50">
                                    <a href="#" className="bg-black text-white rounded-full p-2 m-2">
                                        <ShoppingCartIcon className="h-6 w-6" /> {/* أيقونة السلة */}
                                    </a>
                                    <a href="#" className="bg-black text-white rounded-full p-2 m-2">
                                        <EyeIcon className="h-6 w-6" /> {/* أيقونة المشاهدة */}
                                    </a>
                                </div>
                            </div>
                            <div className="p-4">
